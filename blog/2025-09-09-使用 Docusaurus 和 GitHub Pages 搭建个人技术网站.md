...
-   **Git**：用于版本控制和部署。请确保已安装 Git 并配置好 GitHub 的 SSH 密钥。
-   **Node.js**：你可以在终端中运行 `node -v` 来检查。
...
-   **博客文章**：保存在 `/blog` 目录，使用 Markdown 格式。文件命名通常包含日期，如 `2025-09-09-my-first-blog.md`。
...
3.  **创建GitHub仓库**：在 GitHub 中创建名字为`yourusername.github.io`仓库的
4.  **启用 Pages**：在 GitHub 仓库的 **Settings > Pages** 中，选择 **Source** 为 **GitHub Actions** 。
5.  **推送代码**：将代码推送到 GitHub 的 `main` 分支，Action 会自动运行。

---

## 6. 访问网站

部署成功后，你的网站通常可以通过 `https://yourusername.github.io`（如果仓库名为 `用户名.github.io`）或 `https://yourusername.github.io/<仓库名>`（如果仓库名为其他）访问。
...
2.  **GitHub Actions 部署失败**：  
    检查 Workflow 文件中指定**Source**是否为**GitHub Actions**。。
